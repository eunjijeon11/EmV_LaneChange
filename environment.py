...
def is_near_emv(vid, emv_id, all_vehicles):
    """캐시된 차량 정보로 근접 여부 확인"""
    try:
        return abs(all_vehicles[vid]['pos'] - all_vehicles[emv_id]['pos']) < RADIUS
    except:
        return False
...
class MultiCarEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        for vid, info in all_vehicles.items():
            ...
            if self.emv_mode and EMV_ID in all_vehicles and is_near_emv(vid, EMV_ID, all_vehicles):
    ...
    def _get_rewards(self):
        ...
        for vid, vinfo in all_vehicles.items():
            ...
            if emv_exists:
                ...
                elif not same_lane and -30 < dist_to_emv < 0:
